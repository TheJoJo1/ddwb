<div class="devices-create">
    <div class="page-header">
        <h1 class="page-title">Gerät erstellen</h1>
        <p class="page-subtitle">Neues Gerät zum Inventar hinzufügen</p>
    </div>

    <form method="POST" action="<?= route('devices.store') ?>" class="form" enctype="multipart/form-data">
        <?= csrf_field() ?>
        ...
        <div class="form-grid">
            ...
            <div class="form-grid-column">
                <!-- Internal ID -->
                <div class="form-group <?= has_error('internal_id', $errors ?? []) ? 'form-group-error' : '' ?>">
                    <label for="internal_id" class="form-label required">Interne ID</label>
                    <div class="form-input-wrapper">
                        <input 
                            ...
                            class="form-input"
                            placeholder="DEV-001"
                            value="<?= e($old['internal_id'] ?? $nextInternalId) ?>"
                            required
                        >
                    </div>
                    <?php if (has_error('internal_id', $errors ?? [])): ?>
                        <div class="form-error-message"><?= e(get_error('internal_id', $errors ?? [])) ?></div>
                    <?php endif; ?>
                </div>
                    ...
                    <select id="category_id" name="category_id" class="form-select">
                        ...
                        <?php foreach ($categoryOptions as $option): ?>
                            <option 
                                ...
                                <?= (($old['category_id'] ?? null) === (int)$option['value']) ? 'selected' : '' ?>
                            >
                                <?= e($option['label']) ?>
                            </option>
                        <?php endforeach; ?>
